...
/**
 * Equation calculates Extraterrestrial radiation. Solar radiation incident outside the earth's
 ...
 */
export function getExtraterrestrialIrradiation(
  date,
  lat,
  lon,
  sunConstant = 1367.0
...
function degrees_to_rad(degrees) {
  return (degrees * Math.PI) / 180;
}
function get_declination(dayOfYear) {
  return 23.45 * Math.sin(((2 * Math.PI) / 365.0) * (dayOfYear - 81));
}
function get_hour_angle(date, lon) {
  let solar_time = get_solar_time(date, lon);
  return 15.0 * (solar_time - 12.0);
}

function get_solar_time(date, lon) {
  return (
    (date.getUTCHours() * 60 +
      date.getUTCMinutes() +
      4 * lon +
...
/**
 * Generates Sun radiation applying cloudiness. Cloudiness has to be in the bounds of [0-8]. Normally, it is given by a code in Oktas.
 * This is the sun radiation applying cloud coverage factor.
 ...
 */
export function getGlobalIrradiance(extraterrestrialRadiation, cloudiness) {
 ...
 */
export function getDiffuseIrradiance(
  globalIrradiance,
 ...
 */
export function getDirectIrradiance(globalIrradiance, diffuseIrradiance) {
